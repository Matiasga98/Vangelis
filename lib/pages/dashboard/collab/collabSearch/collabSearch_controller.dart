...
class CollabSearchController extends GetxController {
  ...
  List<CollabCard> collabs = [
    CollabCard(
        open: true,
        finalImage: "images/Hernan.png",
        name: "Hernan Ezequiel Rodriguez Cary",
        description: "Bandoneonista",
        address: "Munro",
        instruments: ["Bandoneon"],
        genres: ["Rock", "Folclore", "Chamame"],
        collabInstrument: "Bateria"),
    CollabCard(
        open: true,
        finalImage: "images/Indio Diego.png",
        name: "Diego Daniel Gagliardi",
        description: "Baterista",
        address: "Ituzaingo",
        instruments: ["Bateria"],
        genres: ["Rock", "Metal", "Pop"],
        collabInstrument: "Piano"),
    CollabCard(
        open: true,
        finalImage: "images/Mantecolati.png",
        name: "Matias Gamal Laye Berardi",
        description: "Pianista",
        address: "Villa del Parque",
        instruments: ["Piano"],
        genres: ["Rock", "Metal", "Jazz"],
        collabInstrument: "Bandoneon")
  ];
  ...
  Future<void> openContext() async {
    filteredMusicians = [];
    List<Instrument> filteredInstruments = wholeInstruments
        .where((element) => selectedInstruments.contains(element.name))
        .toList();
    List<Genre> filteredGenres = wholeGenres
        .where((element) => selectedGenres.contains(element.name))
        .toList();

    List<int> genresIds = filteredGenres.map((g) => g.id).toList();
    List<int> instrumentsIds = filteredInstruments.map((i) => i.id).toList();
    filteredCollabs =
        await collabService.searchCollabs(genresIds, instrumentsIds, false);

    filteredCollabCards = filteredCollabs
        .map((e) => CollabCard(
            open: e.isOpen,
            finalImage: e.musician.userAvatar,
            name: e.musician.userName,
            description: e.description,
            address: "CABA",
            instruments: e.instruments.map((i) => i.name).toList(),
            genres: e.genres.map((g) => g.name).toList(),
            collabInstrument: "instrumento"))
        .toList();
  ...
  bool _filterInstrumentList(CollabCard collab) {
    return selectedInstruments.length <= 0
        ? true
        : collab.instruments
            .any((instrument) => selectedInstruments.contains(instrument));
  }
                  ...
                  CustomButton(
                      ...
                      onTap: () {
                        if (selectedVideo.id != null)
                          Get.to(VideoScreen(
                              filteredCollabs[index].videoId,
                              selectedVideo.id!.videoId!,
                              filteredCollabs[index].id,
                              true));
                        else
                          showMsg(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      border: selectedIndex.value == index
                          ? Border.all(width: 4.0, color: Colors.lightBlue)
